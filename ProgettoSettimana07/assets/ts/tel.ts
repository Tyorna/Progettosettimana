let minutiChiamta: number;
let costo: number;
let nChiamate:number = 0;
...
ricarica(): void {
  ...
  chiamata(): void {
    ...
    if (i == this.numeroChiamate){
        ...
        console.log(`Costo della singola chiamata: ${costo}`);
        this.numero404();
     console.log(`Credito residuo: ${this.numero404().toFixed(2)}`);
     console.log (`Numero chiamate ${nChiamate}`)
     this.azzeraChiamate();
    } else {
        minutiChiamta = Math.floor(Math.random()*15);
        console.log(`${i}° chiamata: ${minutiChiamta} min`);
...
let Utente = new Utenti(40, 6);
let Utente2 = new Utenti(30, 12);
let Utente3 = new Utenti(19, 2);
